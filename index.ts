...
import {quirkyAgent} from "./quirky-agent.ts";
...
const tools = [
    ...
    new DynamicStructuredTool({
        name: "ask-human",
        description: "Ask human for additional information",
        schema: z.object({
...
async function conversation() {
    ...
    if (answers.answer === "exit") {
        ...
        const proxyResp = await chatAgent(answers.answer);

        const res = await agentExecutor.invoke({
            input: answers.answer,
            chat_history: chatHistory,
            agent_scratchpad: proxyResp,
        });

        // const proxyResp = await quirkyAgent(answers.answer, res.output);
